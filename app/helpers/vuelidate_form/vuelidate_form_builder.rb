...
module VuelidateForm; class VuelidateFormBuilder < ActionView::Helpers::FormBuilder
																							...
																							(field_helpers - [:fields_for, :fields, :label, :check_box, :hidden_field, :password_field, :range_field,
		...
		RUBY_EVAL
																							...
																							def range_field(attribute, **options)
																								...
																								options[:step] ||= 1
																								field_builder(attribute, options).field do
																									@template.render 'forms/range_field', options: options, attribute: attribute, object_name: @object_name,
																																																															desc_id: desc_id, t_key: "helpers.sliders.#{@object_name}.#{attribute}" do
																										super
																									end
																								end
																							end
																							...
																							def date_field(attribute, **options)
																								...
																								date_field_builder.field do
																									...
																									@template.content_tag(:'calendar-explainer',
																																															{ ':show-instructions': "#{date_field_builder.slot_scope}.focusedOnly",
																																																	'v-on' => "#{date_field_builder.slot_scope}.$listeners", role: 'combobox' }) do
																										@template.content_tag(:"v-date-picker", '',
																																																{ ':value' => mdl, 'slot-scope': 'explainer', 'v-on': 'explainer.calendarListeners', ':popover' => "{visibility: 'focus'}", ref: 'datepicker' }) do
																							...
																							def password_toggle(toggle_class = nil)
																								...
																								@template.content_tag(:div, { class: 'additional', slot: 'additional' }) do
																									@template.content_tag(:p) do
																										toggle_tag(:password,
																																					**{ class: "link link--no-line #{add_class}", :":symbols" => "['hide_password', 'show_password']",
																																									:":translate" => true, :":vals" => "['text', 'password']", start_val: 'password' })
																									end
																								end
																							end

																							def form_errors
																								@template.content_tag('form-errors',
																																														{ :":submission-error" => "#{SLOT_SCOPE}.submissionError" }) do
																							...
																							def label(attribute, text = nil, options = {}, &block)
																								Tags::Label.new(@object_name, attribute, @template, text,
																																								objectify_options(options)).render(&block)
																							end
																							...
																							def objectify_options(options)
																								super.except(:label, :validate, :show_toggle, :"v-show", :show_if, :tooltip, :label_last,
																																					:is_step, :parent_builder)
																							end
																							...
																							def add_validation(attribute, attr_validators)
																								...
																								@options[:parent_builder].add_nested(validations, @object_name,
																																																													:validations) if @options[:parent_builder]
																							end
