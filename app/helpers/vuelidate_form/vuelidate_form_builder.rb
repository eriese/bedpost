...
module VuelidateForm; class VuelidateFormBuilder < ActionView::Helpers::FormBuilder
	...
	def radio_group(attribute, buttons: [[true], [false]], options: {})
		...
		def range_field(attribute, **options)
			...
			options[:step] ||= 1
			field_builder(attribute, options).field do
				@template.render 'forms/range_field', {
					options: options,
					attribute: attribute,
		...
		date_field_builder.field do
			...
			@template.content_tag(:'calendar-explainer',
				...
				@template.content_tag(:"v-date-picker", '',
					{ 'v-model' => mdl, 'slot-scope': 'explainer', 'v-on': 'explainer.calendarListeners', ':popover' => '{visibility: \'focus\'}', ref: 'datepicker' }) do
					options[:"slot-scope"] = 'dp'
					options[:":value"] = 'dp.inputValue'
