...
Mailjet.configure do |config|
	...
	keys = Rails.env.production? ? Rails.application.credentials.mailjet! : Rails.application.credentials.mailjet || {}
	config.api_key = Rails.env.production? ? keys.fetch(:api_key) {raise 'no mailjet api key found'} : keys[:api_key]
	config.secret_key = Rails.env.production? ? keys.fetch(:secret_key) {raise 'no mailjet secret key found'} : keys[:secret_key]
