...
RSpec.describe VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder, type: :helper do
	...
	def stub_builder(options: {}, validators: [], submission_error: {}, attribute: :name, form_options: {})
		...
		VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder.new(attribute, options, @f_builder, helper)
	end
	...
	describe 'attributes added to the input' do
		...
		describe 'aria-invalid' do
			...
			it 'adds an invalid attribute that vue/vuelidate can change dynamically' do
				...
				expect(get_invalid(true)).to eq "#{VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder::SLOT_SCOPE}.ariaInvalid"
			end
		...
		describe "aria-required" do
			...
			it 'adds a required attribute that vue/vuelidate can change dynamically' do
				...
				expect(get_required(true)).to eq "#{VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder::SLOT_SCOPE}.ariaRequired"
			end
		...
		describe 'vue attributes' do
			it 'sets the v-model option to the model of the field' do
				...
				expect(get_input_options[:"v-model"]).to eq "#{VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder::SLOT_SCOPE}.model"
			end
			...
			it 'sets the blur option if the field validates' do
				@builder = stub_builder options: {validate: true}
				expect(get_input_options[:@blur]).to eq "#{VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder::SLOT_SCOPE}.onBlur"
			end
			...
			it 'sets the focus option if the field validates' do
				@builder = stub_builder options: {validate: true}
				expect(get_input_options[:@focus]).to eq "#{VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder::SLOT_SCOPE}.onFocus"
			end
	...
	describe '#field' do
		...
		context '@input_options' do
			it 'a block given to the field does not still have any of the options meant for the field' do
				...
				@builder = VuelidateForm::VuelidateFormBuilder::NewVuelidateFieldBuilder.new(@attr, opts, @f_builder, helper)
